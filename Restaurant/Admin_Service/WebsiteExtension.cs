...
namespace Admin.Services
{
    public static class WebsiteExtension
    {
        ...
        {
            bool au = false;
            int restaurantId = int.Parse(HttpContext.Current.Application["CompanyId"].ToString());
            int brandId = int.Parse(HttpContext.Current.Application["BranchId"].ToString());
            ...
            //Restaurant Admin Group Id = 2
            if (typeId == 2)
            {
                Per = Admin_Group_Permission_View00.Query("Where GroupId=@0" +
                " AND Status=1", typeId, brandId).ToList();
            }
            else if (restaurantId > 0 && brandId > 0)
            {
                Per = Admin_Group_Permission_View00.Query("Where GroupId=@0 And RestaurantId=@1" +
                " AND Status=1", typeId, restaurantId).ToList();
            }
