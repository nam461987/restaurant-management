...
app.controller("ListController",
    ...
    function ($scope, popupService, $window, $location, Config, svcCache, svcQueue, Model) {
        ...
        $scope.define = function (id) {
            $location.path('/define/' + id);
        };
        ...
        $scope.delete = function (id, status) {
            ...
            $scope.applySuccess = Model.DeleteMenu({
                obj: obj
            }, function (response) {
                ...
                if ($scope.condition.hasOwnProperty(k)) {
                    switch (k) {
                        case "Keyword":
                            ...
                            break;
                        default:
                            if ($scope.condition[k].Value != null && $scope.condition[k].Value != "0") {
                                obj._c[k] = $scope.condition[k].Value;
                            }
                            else if ($scope.condition[k].Value == null && $scope.condition[k] != "0") {
                                obj._c[k] = $scope.condition[k];
                            }
                            break;
                    }
                }
            }
            Model.GetAllMenu({ obj: obj }, function (response) {
                if (response.Result == 1) {
...
app.controller("CreateController",
    ...
    function ($scope, $location, svcCache, svcQueue, Config, Model, Upload) {
        ...
        $scope.ok = function () {
            ...
            $scope.applySuccess = Model.InsertMenu({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $location.path('/');
...
app.controller("UpdateController",
    ...
    function ($scope, $stateParams, svcCache, svcQueue, $location, Config, Model,Upload) {
        ...
        $scope.ok = function () {
            ...
            $scope.applySuccess = Model.UpdateMenu({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $location.path('/');
...
app.controller("DefineController",

    function ($scope, $stateParams, popupService, $window, $location, svcCache, svcQueue, Config, Model) {

        // Create action part
        $scope.record = {};

        $scope.createFields = $.map(Config.fieldsSecond, function (f) {
            if (f.create === true) {
                //$scope.record.MadeTime = moment().format("MM/DD/YYYY HH:mm");
                return f;
            }
        ...
        $scope.resetForm = function () {
            for (var j = 0; j < Config.fieldsSecond.length; j++) {
                var f = Config.fieldsSecond[j];
                if (typeof $scope.record[f.field] != "undefined") {
                    switch (f.type) {
                        case 'date':
                        case "datetime":
                        case "time":
                        case "textarea":
                            $scope.record[f.field] = '';
                            break;
                        case "select":
                            $scope.record[f.field].Value = 0;
                            break;
                        case "select2":
                            $scope.record[f.field] = 0;
                            break;
        ...
        $scope.ok = function () {
            ...
            //Set MenuId 
            obj.MenuId = $stateParams.id;

            $scope.applySuccess = Model.InsertMenu_Definition({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $scope.reload();
        ...
        $scope.delete = function (id, status) {
            ...
            $scope.applySuccess = Model.DeleteMenu_Definition({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $scope.reload();
        ...
        $scope.reload = function () {
            ...
            Model.GetAllMenu_Definition({ obj: obj }, function (response) {
                if (response.Result == 1) {
                    $scope.records = response.Records;
                    $scope.totalRecord = response.TotalRecordCount;
                    $('.overlay').addClass('hidden');
        ...
        $scope.listFields = $.map(Config.fieldsSecond, function (f) {
            if (f.list === true) {
                return f;
            }
            return null;
        ...
        $scope.$watch(function () { return svcCache.get('appReady'); }, function (newValue, oldValue, scope) {
            if (newValue === true || oldValue === false) {
                for (var i = 0; i < Config.fieldsSecond.length; i++) {
                    var f = Config.fieldsSecond[i];
                    if (f.type == "select" || f.type == "select2") {
                        scope.options[f.field] = [];
                        //var cacheKey = 'options_' + f.field + '_' + f.option;
...
app.controller("Update_Menu_DefineController",

    function ($scope, $stateParams, svcCache, svcQueue, $location, Config, Model) {
        $scope.record = {};

        $scope.fields = $.map(Config.fieldsSecond, function (f) {
            if (f.edit === true) {
                return f;
            }
            return null;
        ...
        $scope.back = function () {
            $location.path('define/' + $scope.record.MenuId);
        };
        ...
        $scope.ok = function () {
            ...
            $scope.applySuccess = Model.UpdateMenu_Definition({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $location.path('define/' + $scope.record.MenuId);
