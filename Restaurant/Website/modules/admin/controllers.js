...
var app = angular.module('ngService.controllers', []);
...
app.controller('ListCtrl', ['$scope', 'Config', 'Factory', '$location', 'svcCache', 'svcQueue',
    ...
    function ($scope, Config, Factory, $location, svcCache, svcQueue) {
        ...
        $scope.currentAccType = globalGroupId;
    ...
    function ($scope, Config, Factory, $location, svcCache, svcQueue) {
        if (true == true) {
            ...
            $scope.insert = function () {
                if ($scope.record.UserName.length < 8 || !checkSpecialCharacter($scope.record.UserName || $scope.record.PasswordHash.length < 6)) {
                    alert("Username must be at least 8 characters "
                        + " \n Do not have special characters"
                        +" \n Password length is at least 6 characters");
                }
                else {
                    Factory.checkExist($scope.record.UserName).then(function (data) {
                        if (data) {
