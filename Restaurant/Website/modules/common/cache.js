...
var cache = angular.module('gcApp.caches', []);
...
cache.run([
    'Config', 'svcCache', 'svcQueue', function (Config, svcCache, svcQueue) {
                ...
                switch (typeof Config.fields[i].option) {
                    case "string":
                        ...
                        if (typeof (optcache) == "undefined") {
                            oqKey.push(cacheKey);
                            svcQueue({ method: "POST", url: Config.fields[i].option }).then(function (result) {
        ...
        if (Config.fieldsSecond != null) {
            for (var i = 0; i < Config.fieldsSecond.length; i++) {
                if (typeof Config.fieldsSecond[i].option != "undefined") {
                    switch (typeof Config.fieldsSecond[i].option) {
                        case "string":
                            var cacheKey = 'options_' + Config.fieldsSecond[i].field + '_' + Config.fieldsSecond[i].option;
                            var optcache = svcCache.get(cacheKey);
                            if (typeof (optcache) == "undefined") {
                                oqKey.push(cacheKey);
                                svcQueue({ method: "POST", url: Config.fieldsSecond[i].option }).then(function (result) {
                                    var odata = result.data;
                                    if (odata.Result == 1) {
                                        var ridx = oqKey.shift();
                                        svcCache.put(ridx, odata.Records); //Store to cache
                        ...
                        case "object":
                            var cacheKey2 = 'options_' + Config.fieldsSecond[i].field + '_array';
                            var optcache2 = svcCache.get(cacheKey2);
                            if (typeof (optcache2) == "undefined") {
                                svcCache.put(cacheKey2, Config.fieldsSecond[i].option); //Store to cache
                            }
