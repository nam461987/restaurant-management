...
app.controller("ListController",
    ...
    function ($scope, popupService, $window, $location, Config, svcCache, svcQueue, Model) {
        ...
        $scope.delete = function (id, status) {
            ...
            $scope.applySuccess = Model.DeleteRestaurant_Branch({
                obj: obj
            }, function (response) {
        ...
        $scope.reload = function () {
                    ...
                    switch (k) {
                        case "Keyword":
                            if ($scope.condition[k].length > 3) {
                                obj._c["Name"] = "PATINDEX('%{0}%',Name) > 0 OR PATINDEX('%{0}%',Address) > 0 "
                                    + " OR PATINDEX('%{0}%', Phone) > 0".format($scope.condition[k]);
                            }
                            break;
                        default:
            ...
            Model.GetAllRestaurant_Branch({ obj: obj }, function (response) {
                if (response.Result == 1) {
...
app.controller("CreateController",
    ...
    function ($scope, $location, svcCache, svcQueue, Config, Model) {
        ...
        $scope.ok = function () {
            ...
            $scope.applySuccess = Model.InsertRestaurant_Branch({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $location.path('/');
...
app.controller("UpdateController",
    ...
    function ($scope, $stateParams, svcCache, svcQueue, $location, Config, Model) {
        ...
        $scope.ok = function () {
            ...
            $scope.applySuccess = Model.UpdateRestaurant_Branch({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $location.path('/');
        ...
        Model.GetAllRestaurant_Branch({ obj: obj }, function (response) {
            if (response.Result == 1) {
                ...
                $scope.record.AllDay = $scope.AllDay[response.Records[0].AllDay];
            }
        });
    });