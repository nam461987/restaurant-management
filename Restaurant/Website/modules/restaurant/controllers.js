...
app.controller("ListController",
    ...
    function ($scope, popupService, $window, $location, Config, svcCache, svcQueue, Model) {
        ...
        $scope.delete = function (id, status) {
            ...
            $scope.applySuccess = Model.DeleteRestaurant({
                obj: obj
            }, function (response) {
        ...
        $scope.reload = function () {
                    ...
                    switch (k) {
                        case "Keyword":
                            if ($scope.condition[k].length > 3) {
                                obj._c["Name"] = "PATINDEX('%{0}%',Name) > 0 OR PATINDEX('%{0}%',Description) > 0 "
            ...
            Model.GetAllRestaurant({ obj: obj }, function (response) {
                if (response.Result == 1) {
...
app.controller("CreateController",
    ...
    function ($scope, $location, svcCache, svcQueue, Config, Model) {
        ...
        $scope.ok = function () {
            ...
            $scope.applySuccess = Model.InsertRestaurant({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $location.path('/');
...
app.controller("UpdateController",
    ...
    function ($scope, $stateParams, svcCache, svcQueue, $location, Config, Model) {
        ...
        $scope.ok = function () {
            ...
            $scope.applySuccess = Model.UpdateRestaurant({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $location.path('/');
